...
public class BiddingUI : MonoBehaviour
{
    ...
    void Start()
    {
        // Find GameStage reference
        m_stage = FindObjectOfType<GameStage>();
    ...
    void SetupButtonListeners()
    {
        ...
        if (diamondsButton != null)
            diamondsButton.onClick.AddListener(() => SubmitBid(Bid.CreateBid(Card32Family.Diamonds)));

        if (heartsButton != null)
            heartsButton.onClick.AddListener(() => SubmitBid(Bid.CreateBid(Card32Family.Hearts)));

        if (spadesButton != null)
            spadesButton.onClick.AddListener(() => SubmitBid(Bid.CreateBid(Card32Family.Spades)));
    }
    ...
    void OnGUI()
    {
        ...
        if (m_isBiddingActive && m_showBiddingUI)
        {
            ...
            if (m_currentBidder is HumanPlayer)
            {
                ...
                {
                    SubmitBid(Bid.CreateBid(Card32Family.Diamonds));
                }
                
                if (GUI.Button(new Rect(20, 140, 60, 30), "Hearts"))
                {
                    SubmitBid(Bid.CreateBid(Card32Family.Hearts));
                }
                
                if (GUI.Button(new Rect(90, 140, 60, 30), "Spades"))
                {
                    SubmitBid(Bid.CreateBid(Card32Family.Spades));
                }
            }
            else
            {
