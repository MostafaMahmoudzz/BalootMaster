...
public class BelootBiddingSystem
{
    ...
    public Player CurrentBidder
    {
        get
        {
            if (m_biddingOrder != null && m_currentBidderIndex < m_biddingOrder.Count)
            {
                ...
                // Debug: Log EVERY time CurrentBidder is accessed to track the issue
                Debug.Log($"[BiddingSystem] CurrentBidder accessed: {currentBidder?.Name} (index: {m_currentBidderIndex}, order count: {m_biddingOrder.Count})");
                
                // Debug: Show stack trace to see who's calling this
                System.Diagnostics.StackTrace stackTrace = new System.Diagnostics.StackTrace();
                Debug.Log($"[BiddingSystem] CurrentBidder called from: {stackTrace.GetFrame(1).GetMethod().Name}");
    ...
    private void CheckBiddingComplete()
    {
        ...
        // For Round 2, we need to consider Trump confirmation and Sun declaration
        bool hasActualBidInCurrentRound = hasActualBid;
        if (m_currentRound == BiddingRound.Round2)
        {
            // In Round 2, we also consider Trump confirmation and Sun declaration as "actual bids"
            if (m_trumpConfirmed || m_sunDeclared)
            {
                hasActualBid = true;
                hasActualBidInCurrentRound = true; // IMPORTANT: Also set this for Round 2 completion logic
                Debug.Log($"[BiddingSystem] Round 2: Considering Trump confirmed ({m_trumpConfirmed}) or Sun declared ({m_sunDeclared}) as actual bid");
            }
        }
        ...
        if (bidsSubmitted >= m_biddingOrder.Count)
        {
            ...
            else if (m_currentRound == BiddingRound.Round2)
            {
                ...
                {
                    Debug.Log("[BiddingSystem] Both rounds had no Trump/Sun bids - DISABLED: would normally start new dealer and new round.");
                    Debug.Log("[BiddingSystem] TEMPORARILY DISABLED FOR ANOTHER TRUMP DEBUGGING - staying in current bidding state");
                    // DISABLED FOR DEBUGGING: m_biddingComplete = true; 
                    // DISABLED FOR DEBUGGING: SendNoBidsEvent();
                }
                else
                {
                ...
                else if (m_anotherTrumpChosen)
                {
                    ...
                    m_biddingComplete = true; // IMPORTANT: Set bidding complete so system knows round is finished
    ...
    private void StartRound2()
    {
        ...
        // Reset first bidder and Trump confirmation for Round 2
        m_firstBidder = null;
        m_trumpConfirmed = false;
        m_waitingForTrumpSuitSelection = false;
    ...
    public void SelectTrumpSuit(Card32Family trumpSuit)
    {
        ...
        // Finalize the bidding (biddingComplete is already true from Round 2 completion)
        Debug.Log("[BiddingSystem] Finalizing bidding with selected trump suit");
        FinalizeBidding();
    }
