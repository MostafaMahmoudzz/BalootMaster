...
public class BelootBiddingSystem
{
    ...
    public void StartBidding(List<Player> players, Player firstBidder, BeloteCard faceUpCard)
    {
        ...
        // Debug: Show the bidding order
        Debug.Log($"[BiddingSystem] Bidding order:");
        for (int i = 0; i < m_biddingOrder.Count; i++)
        {
            Debug.Log($"[BiddingSystem]   {i}: {m_biddingOrder[i].Name}");
        }
        
        // Find first bidder index
        m_currentBidderIndex = m_biddingOrder.IndexOf(firstBidder);
        Debug.Log($"[BiddingSystem] First bidder {firstBidder?.Name} found at index: {m_currentBidderIndex}");
        ...
        Debug.Log($"[BiddingSystem] About to send BiddingStartEvent...");
