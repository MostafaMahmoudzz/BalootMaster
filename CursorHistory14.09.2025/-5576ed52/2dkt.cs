...
public class GameStageRenderer
{
    ...
    {
        // Hide face-up card after bidding (before cards are dealt)
        UnSpawnFaceUpCard();
        
        // Show cards after bidding is complete
        SpawnCards();
        Refresh();
    }
    ...
    protected void SpawnFaceUpCard()
    {
        if (Stage.FaceUpCard != null)
        {
            CardComponent faceUpCardComponent = Stage.FaceUpCard.Spawn();
            if (faceUpCardComponent != null)
            {
                ...
                faceUpCardComponent.SetInitialPosition(faceUpPosition);
                
                // Make it slightly larger and elevated to stand out
                faceUpCardComponent.transform.localScale = new Vector3(1.2f, 1.2f, 1.0f);
                faceUpCardComponent.transform.position = new Vector3(faceUpPosition.x, faceUpPosition.y, -2);
                
                m_cards.Add(faceUpCardComponent); // Track the face-up card
            }
        }
    }
    ...
    {
        // Find and remove the face-up card component
        // We need to find it by checking if it's a card with no owner (face-up cards have null owner)
        CardComponent faceUpCardComponent = null;
        foreach (CardComponent cardComp in m_cards)
        {
            if (cardComp.Card != null && cardComp.Card.Owner == null)
            {
                faceUpCardComponent = cardComp;
                break;
            }
        }
        ...
        if (faceUpCardComponent != null)
        {
            UnSpawnCard(faceUpCardComponent);
        }
    }
