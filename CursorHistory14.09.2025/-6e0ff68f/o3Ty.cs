...
public class GameStage : Stage, IDeckOwner
{
    ...
    void OnBiddingNoBids(BiddingNoBidsEvent evt)
    {
        if (evt.BothRoundsNoBids)
        {
            ...
            // IMPORTANT: Set ignore flag in BiddingUI to prevent old BiddingTurnEvent from interfering
            Debug.Log("[GameStage] Setting ignore flag in BiddingUI to prevent old BiddingTurnEvent from interfering");
            
            // Get the BiddingUI component and set the ignore flag
            BiddingUI biddingUI = GetComponent<BiddingUI>();
            if (biddingUI != null)
            {
                biddingUI.SetIgnoreBiddingTurnEvents(true);
            }
            else
            {
                Debug.LogWarning("[GameStage] Could not find BiddingUI component to set ignore flag");
            }
