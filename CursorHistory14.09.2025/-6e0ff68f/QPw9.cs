...
public class GameStage : Stage, IDeckOwner
{
    ...
    void OnBiddingNoBids(BiddingNoBidsEvent evt)
    {
        if (evt.BothRoundsNoBids)
        {
            ...
            // Send event to set ignore flag in BiddingUI
            BiddingTurnEventIgnoreEvent ignoreEvt = Pools.Claim<BiddingTurnEventIgnoreEvent>();
            ignoreEvt.Ignore = true;
            GameEventDispatcher.SendEvent(ignoreEvt);
