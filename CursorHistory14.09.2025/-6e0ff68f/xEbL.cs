...
public class GameStage : Stage, IDeckOwner
{
    ...
    //-------------------------------------------------------
    private void DealCardsForTrumpContract(Player trumpContractMaker, BeloteCard faceUpCard)
    {
        Debug.Log($"[GameStage] === DEALING CARDS FOR TRUMP CONTRACT ===");
        Debug.Log($"[GameStage] Trump contract maker: {trumpContractMaker.Name}");
        Debug.Log($"[GameStage] Face-up card: {(faceUpCard != null ? $"{faceUpCard.Value} of {faceUpCard.Family}" : "None")}");
        
        // Trump contract maker gets the face-up card + 2 additional cards (total 8)
        if (faceUpCard != null)
        {
            trumpContractMaker.Hand.AddCard(faceUpCard);           // Give face-up card to Trump contract maker
            faceUpCard.Owner = trumpContractMaker;                 // Set ownership
            Debug.Log($"[GameStage] {trumpContractMaker.Name} receives face-up card: {faceUpCard.Value} of {faceUpCard.Family}");
        }
        
        // Deal 2 additional cards to Trump contract maker (since they already got the face-up card)
        m_deck.MoveCardsTo(2, trumpContractMaker.Hand);
        Debug.Log($"[GameStage] {trumpContractMaker.Name} receives 2 additional cards (total: {trumpContractMaker.Hand.Size} cards)");
        
        // Deal 3 cards to all other players
        foreach (Player player in m_players)
        {
            if (player != trumpContractMaker)
            {
                m_deck.MoveCardsTo(3, player.Hand);
        ...
        Debug.Log($"[GameStage] Trump contract completed - {trumpContractMaker.Name} has {trumpContractMaker.Hand.Size} cards total");
    }
    ...
    void OnBiddingComplete(BiddingCompleteEvent evt)
    {
        if (evt.WinningBidder != null && evt.WinningBid != null)
        {
            ...
            {
                // Trump contract: Contract maker gets face-up card + 2 additional cards, others get 3 cards
                DealCardsForTrumpContract(evt.WinningBidder, faceUpCardToClean);
            }
