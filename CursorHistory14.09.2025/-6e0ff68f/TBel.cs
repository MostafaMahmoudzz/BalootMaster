...
public class GameStage : Stage, IDeckOwner
{
    ...
    void OnBiddingNoBids(BiddingNoBidsEvent evt)
    {
        if (evt.BothRoundsNoBids)
        {
            ...
            // IMPORTANT: Temporarily unsubscribe from BiddingTurnEvent to prevent old events from interfering
            Debug.Log("[GameStage] Temporarily unsubscribing from BiddingTurnEvent to prevent old events from interfering");
            
            // Temporarily unsubscribe UI components from BiddingTurnEvent
            if (m_renderer != null)
            {
                // Note: We can't directly unsubscribe from GameStageRenderer here as it's not accessible
                // The BiddingUI will handle this through its own subscription management
            }
            
            // Reset bidding system FIRST to prevent old events from interfering
            Debug.Log("[GameStage] Resetting bidding system to prevent old events from interfering");
            m_biddingSystem.Reset();
