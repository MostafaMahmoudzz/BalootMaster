...
public class GameStage : Stage, IDeckOwner
{
    ...
    private void ValidateDeckIntegrity()
    {
        ...
        // Calculate expected deck size based on game state
        // If no contract was made, we should have 21 cards (5 per player + 1 face-up)
        // If contract was made, we should have 32 cards (8 per player + 1 face-up, but face-up is given to contract maker)
        int expectedDeckSize = 32; // Default for full deck
        
        // If this is after no contract, we should have fewer cards
        if (m_deck.Size == 21)
        {
            expectedDeckSize = 21; // 5 cards per player + 1 face-up = 21 total
            Debug.Log("[GameStage] No contract scenario detected - expecting 21 cards (5 per player + 1 face-up)");
        }
        
        if (m_deck.Size != expectedDeckSize)
        {
            Debug.LogError($"[GameStage] Deck integrity check failed: Expected {expectedDeckSize} cards, got {m_deck.Size}");
            return;
        }
