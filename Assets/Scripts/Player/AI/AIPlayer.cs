...
public class AIPlayer : Player
{
    ...
    private Bid MakeBidDecision(Bid highestBid)
    {
        ...
        if (biddingSystem.CurrentBiddingRound == BelootBiddingSystem.BiddingRound.BiddingRound1)
        {
            return MakeBiddingRound1Bid(biddingSystem, highestBid);
        }
        else // Round 2
        {
            return MakeBiddingRound2Bid(biddingSystem, highestBid);
        }
    }

    //--------------------------------------------------------------------
    private Bid MakeBiddingRound1Bid(BelootBiddingSystem biddingSystem, Bid highestBid)
    {
        // Round 1 Rules:
    ...
    //--------------------------------------------------------------------
    private Bid MakeBiddingRound2Bid(BelootBiddingSystem biddingSystem, Bid highestBid)
    {
        // Round 2 Rules:
        ...
        Debug.Log($"[AIPlayer] {this.Name} Round 2: IsTrumpTaker = {isTrumpTaker}, AnotherTrumpChosen = {anotherTrumpChosen}");
        ...
        if (isTrumpTaker)
        {
            ...
            if (random < 0.7f)
            {
                ...
                Debug.Log($"[AIPlayer] {this.Name} Round 2: Confirming Trump ({biddingSystem.FaceUpCard.Family})");
                return Bid.CreateTrump(biddingSystem.FaceUpCard.Family);
            }
            else
            {
                // 30% chance to switch to Sun
                Debug.Log($"[AIPlayer] {this.Name} Round 2: Switching to Sun");
                return Bid.CreateSun();
            }
        }
        else if (anotherTrumpChosen)
        {
            ...
            {
                Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Sun (override Another Trump)");
                return Bid.CreateSun();
            }
            else
            {
                Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Pass (Another Trump chosen)");
                return Bid.CreatePass();
            }
        }
        else
        {
            ...
            if (isCaseA)
            {
                ...
                {
                    Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Sun (Case A - override Trump)");
                    return Bid.CreateSun();
                }
                else
                {
                    Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Pass (Case A)");
                    return Bid.CreatePass();
                }
            }
            else
            {
                ...
                if (random < 0.3f)
                {
                    ...
                    Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Sun (Case B)");
                    return Bid.CreateSun();
                }
                else if (random < 0.6f)
                {
                    ...
                    Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Another Trump ({anotherSuit}) (Case B)");
                    return Bid.CreateTrump(anotherSuit);
                }
                else
                {
                    // 40% chance to pass
                    Debug.Log($"[AIPlayer] {this.Name} Round 2: Choosing Pass (Case B)");
                    return Bid.CreatePass();
                }
            }
        }
